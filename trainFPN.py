"""Train a FPN based GAN"""

from modelFPN import Gen, Discrim
...
## Train
class trainGan():
    def __init__(self, nb_epochs=200, lr=3e-4, savefileGen="netWeights/genFPNHW.pth",
            savefileDiscrim="netWeights/discrimFPNHW.pth", discrimTrainPeriod=2,
            displayPeriod=100):
         #Input/output
         basedir = os.path.dirname(__file__)
         ...
         self.optimizerGen = torch.optim.Adam(self.gen.parameters(), lr=lr, betas=(0.5,
             0.999))
         self.optimizerDiscrim = torch.optim.Adam(self.discrim.parameters(),
